import axios from "axios";
import { useEffect, useState } from "react";


export default function AcceptPermissions() {
  const [user, setUser] = useState([]);
  const [choosen, setChoosen] = useState("");
  const index3 = [];
  

  // const user = [];
  // let user;
  // const getUsers = () => axios.get("http://localhost:8081/hrm/user/getAllLeaves").then((response) => setUser(response.data));

  useEffect(() => {
    // const getUsers = () => axios.get("http://localhost:8081/hrm/user/getAllLeaves").then((response) => setUser(response.data));
    // const getUsers = () => axios.get("http://localhost:8081/hrm/user/getAllLeaves").then((response) => user.push(response.data)
    // )
    //  axios.get("http://localhost:8081/hrm/user/getAllLeaves").then((response) => {
    //    user = response.data
    //  })
    axios
      .get("http://localhost:8081/hrm/user/getAllLeaves")
      .then((response) => {
        setUser(response.data);
      });
  }, []);

  function HandleAccept(index) {
    
    console.log("U", user);
    axios
      .post("http://localhost:8081/hrm/user/acceptLeave/" + user[index].id)
      .then((response) => {
        console.log(response);
      });
    // alert("Permission Accepted")
    
    
    
    
  }

  function HandleDeny(index) {
    axios
      .post("http://localhost:8081/hrm/user/denyLeave/" + user[index].id)
      .then((response) => {
        console.log(response);
      });
    // alert("Permission Denied")
    
    
  }

  function HandleSearch(e) {
    e.preventDefault();
    setChoosen(e.target.value);
  }

  return (
    // <div className="container">
    //   <p className="h2 text-center">Accept Permission Page</p>

    //   {/* {getUsers} */}
    //   <ul className="list-group">
    //   {user.map((user, index) => (
    //     <li className="list-group-item border" key={index}>FirstName: {user.firstName}, LastName: {user.lastName}, Description: {user.description},StartDate: {user.startDate}, EndDate: {user.endDate}
    //     <button type="button" className="btn btn-primary ms-2 me-2" onClick={() =>HandleAccept(index)}>Accept</button>
    //     <button type="button" className="btn btn-danger" onClick={() => HandleDeny(index)}>Deny</button>

    //     </li>

    //   ))}
    //   </ul>

    // </div>
    <>
      <h1
        className="text-center"
        style={{ textAlign: "center", color: "#FFBF00" }}
      >
        Accept Permission Page
      </h1>
      <hr />
      <br />
      <input
        type="search"
        name="search"
        placeholder="Enter a employee"
        aria-label="Search"
        value={choosen}
        onChange={(e) => setChoosen(e.target.value)}
        onSubmit={HandleSearch}
      />
      {/* <table style={{ marginLeft: "55px", marginRight: "auto" }}>
        <thead data-theme="dark">
          <tr>
            <th>FirstName</th>
            <th>LastName</th>
            <th>Description</th>
            <th>StartDate</th>
            <th>EndDate</th>
          </tr>
        </thead> */}

      {user
        .filter((user, index) => {
          if (
            user.firstName.toLowerCase() === choosen.toLocaleLowerCase() ||
            user.lastName.toLowerCase() === choosen.toLocaleLowerCase() 
            
          ) {
            index3.push(index);
            return user;
          }
        })
        .map((user, index) => (
          <>
          <table key={index} style={{ marginLeft: "55px", marginRight: "auto" }}>
            <thead data-theme="dark">
              <tr>
                <th>FirstName</th>
                <th>LastName</th>
                <th>Description</th>
                <th>StartDate</th>
                <th>EndDate</th>
              </tr>
            </thead>
            <tbody>
              <tr key={index}>
                <td>{user.firstName}</td>
                <td>{user.lastName}</td>
                <td>{user.description}</td>
                <td>{user.startDate}</td>
                <td>{user.endDate}</td>
                <td>
                  <button
                    type="button"
                    style={{ backgroundColor: "#1343A0" }}
                    className="btn btn-primary ms-2 me-2"
                    onClick={() => HandleAccept(index3)}
                  >
                    Accept
                  </button>
                  <button
                    style={{ backgroundColor: "#D93526", marginLeft: "10px" }}
                    type="button"
                    className="btn btn-danger"
                    onClick={() => HandleDeny(index3)}
                  >
                    Deny
                  </button>
                </td>
              </tr>
              
            </tbody>
          </table>
          </>
        ))}
    </>
  );
}
